...
class Calendario(tk.Frame, Evento):
    ...
    def mostrar_mes(self):
        ...
        self.root.title(
            calendar.month_name[self.mes_actual] + " " + str(self.año_actual))
    ...
    def mes(self):
        ...
        # ? bg usa un color hexadecimal para dar mejor tonalidad y no estar tan blanco y no muy gris :(xD) noqa

        # ? Fira Code una fuente personalizada para que se vea mas bonito y no tan feo en caso de no tener ninguna fuente segun su documentacion de tkinter toma la defult del sistema #noqa

        for i, dia in enumerate(dias_semana):
            etiqueta = tk.Label(self.root, text=dia, font=(
                "Fira Code", 12), width=10, height=2, bg="#fff", bd=1, relief="solid")
            etiqueta.grid(row=0, column=i, padx=2, pady=2)

            et_dia = tk.Label(self.root)
            # ESTO SOLO MUESTRA LOS NOMBRES NO AGARRA EL DIA EN SI
            et_dia.bind("<Enter>", self.mostrar_dia)
        ...
        for dia in range(1, self.dias_mes + 1):
            etiqueta = tk.Label(self.root, text=dia, font=(
                "Arial", 12), width=10, height=5, bg="#eee", bd=1, relief="solid")
